...
function filterPosts() {
  ...
  for (var i=0; i < imageArray.length; i++) {
    ...
    if (imageArray[i].distance < distanceInput && imageArray[i].time < timeMax && imageArray[i].time > timeMin && (postsToShow == "all" || (postsToShow == "liked" && likedPosts.includes(imageArray[i].id) || (postsToShow == "posted" && imageArray[i].user == userId) || (postsToShow == "followposted" && followedUsers.includes(imageArray[i].user)) || (postsToShow == "followliked" && followLiked.includes(imageArray[i].id)))) && (profileFilter == -1 || imageArray[i].user == profileFilter) && !usersBlockedMe.includes(imageArray[i].user) && (imageArray[i].visibility != "following" || usersFollowingMe.includes(imageArray[i].user) || imageArray[i].user == userId)) {
     //console.log("Pushing Post #" + imageArray[i].id);
      arrayOfPosts.push(imageArray[i]);
    }
    
    if (postMode == "admin") {
        if (imageArray[i].time < timeMax && imageArray[i].time > timeMin && (postsToShow == "all" || (postsToShow == "liked" && likedPosts.includes(imageArray[i].id)) || (postsToShow == "posted" && imageArray[i].user == userId) || (postsToShow == "followposted" && followedUsers.includes(imageArray[i].user)) || (postsToShow == "followliked" && followLiked.includes(imageArray[i].id))) && (profileFilter == -1 || imageArray[i].user == profileFilter)) {
        //console.log("Pushing Post #" + imageArray[i].id);
            arrayOfPosts.push(imageArray[i]);
        }
    }
                ...
                if (xmlhttp.responseText == "noLocation") {
                  ...
                  //console.log("server response: " + xmlhttp.responseText);
                    ...
                    window.addEventListener('scroll', () => {
                      ...
                      if (window.scrollY + window.innerHeight + (0.4 * window.innerHeight) >= document.documentElement.scrollHeight) {
...
function fillPics(jsonInfo) {
        ...
        if (object.hidden != true && !blockedUsers.includes(object.user) && (jsonInfo[counter-1] == null || jsonInfo[counter].id != jsonInfo[counter-1].id) && !deletedPosts.includes(object.id)) {
            ...
            if (profileFilter == -1) feedString += "<p id='profile" + object.id + "' class='userText' onclick='showProfile(" + object.user + ")'>"+ userName + "  <i class='fa-solid fa-user'></i></p>";

            feedString += "<img id='" + "img" + object.id + "' src='./posts/" + object.image + "'> <div id='" + "cap" + object.id + "'class='caption'><div class='capLeft'><p> <span id='locationSpan'><i class='fa-solid fa-location-dot'></i> " + distanceString + "</span> <i class='fa-solid fa-calendar'></i> " + newdate + " </p></div><div class='capRight'><p> <span id='likes" + object.id + "'>" + displayNum(object.likes) + "</span> <i id='heart" + object.id + "' onclick='likePost(" + object.id + ")' class='" + heartClass + " fa-heart'></i>  <i id='flag" + object.id + "' class='fa-solid " + flagClass + "' onclick='flagPost(" + object.id + ")'></i></p></div>";
            
            var styleAlt = "display: none;";
            var commentButtonString = " - Show Comments - ";
            
            if (object.id == openComments) {
                styleAlt = "";
                commentButtonString = " - Hide Comments - ";
            }
            ...
            if (object.comments !== null && object.comments !== undefined) {
                ...
                for (var i=0; i < comments.length; i++) {
                    if (comments[i].hidden != true && !usersBlockedMe.includes(comments[i].user) && !blockedUsers.includes(comments[i].user)) {
                        ...
                        if (username == "Noah") comments[i].userName = comments[i].userName + ' <i class="fa-solid fa-circle-check"></i>';
                        ...
                        feedString += "<div class='commentLeft'><p><span onclick='showProfile(" + comments[i].user + ")' class='commUName'>" + comments[i].userName + ": </span><span class='commentText'>" + comments[i].comment + "</span></p></div><div class='commentRight'>" + deleteButton + "</div>";
            ...
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (response.charAt(0) == '{') {
                        ...
                        document.getElementById("comments" + id).innerHTML  += "<div id='comment" + responseObj.id + "post" + id + "'>" + divide + "<div class='commentLeft'><p>" + uName + ": " + responseObj.comment + "</p></div><div class='commentRight'>" + deleteButton + "</div></div>";
...
function refreshComments() {
    ...
    //for (var i=0; i < openComments.length; i++) {
        ...
        if (openComments != -1) {
            ...
            // Set POST method and ajax file path
            xhttp.open("POST", "./php/getPostsComments.php?_='" + Date.now(), true);
                ...
                if (response.charAt(0) == '[') {
                    ...
                    for (var i=0; i < postsComments.length; i++) {
                        if (postsComments[i].hidden != true && !usersBlockedMe.includes(postsComments[i].user) && !blockedUsers.includes(postsComments[i].user)) {
                            ...
                            if (username == "Noah") postsComments[i].userName = postsComments[i].userName + ' <i class="fa-solid fa-circle-check"></i>';
                            ...
                            feedString += "<div class='commentLeft'><p><span onclick='showProfile(" + postsComments[i].user + ")' class='commUName'>" + postsComments[i].userName + ": </span><span class='commentText'>" + postsComments[i].comment + "</span></p></div><div class='commentRight'>" + deleteButton + "</div>";
                    ...
                    if (displayed == 0) {
                        document.getElementById("comments" + openComments).style.display = "none";
                        document.getElementById("commentsToggle" + openComments).style.display = "none";
                    }
            ...
            refreshTime = setTimeout(function (){ refreshComments(); }, 30000);
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (response == 1) {
                       ...
                       console.log("Post Liked");
                        ...
                        document.getElementById("likeCount").innerHTML = displayNum(likeCount);
...
function updateLikes(postID) {
    ...
    document.getElementById("likes" + postID).innerHTML = displayNum(likePostCount);
...
function showProfile(id) {
    ...
    document.getElementById("profilePostCount").innerHTML = displayNum(arrayOfPosts.length);
    document.getElementById("profilePostCount2").innerHTML = displayNum(postCount);
    ...
    if (followedUsers.includes(id)) {
        ...
        document.getElementById("followButtonProfile").setAttribute("onclick", "followUser("+ id + ")");
    }
