...
if(isset($_POST["id"]) and isset($_POST["session"]) and file_exists("sessions.json")){
                ...
                if ($users[$m]->id == $uID) {
                    ...
                    if ($users[$m]->emailIsVerified) {
                        ...
                        for ($i=0; $i < count($userBlocked); $i++) {
                            if (intval($userBlocked[$i]) == intval($_POST["id"])) $validSession = true;
                        }
    ...
    if ($validSession) {
        ...
        if (($key = array_search(intval($FollowId), $userBlocked)) !== false) {
            unset($userBlocked[$key]);
        }

        #array_push($userBlocked, intval($FollowId));
        
        $users[$userIDStore]->blocked = json_encode(array_values($userBlocked));
        
        $userBlocked = array_values($userBlocked);
        ...
        if (file_put_contents("users.json", json_encode($users))) {
            ...
            $response = json_encode(array_values($userBlocked));
            
            sendMessage("Oh not nice! " . $users[$userIDStore]->user . " just unblocked user #" . $FollowId);
            logAction($users[$userIDStore]->user . " (user id " . $users[$userIDStore]->id . ") unblocked user #" . $FollowId);
        } else {
            $response = 0;
        }
